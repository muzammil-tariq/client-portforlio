...
import { getimages, upload, uiux } from "@/endPoints";
...
const CreateWebsite = () => {
  ...
  const handleUpdateAndNavigate = async () => {
    ...
    router.push(`/admin/ui-ux/view`); // Navigate after state update
  };
  const handleUpdate = (imageLink) => {
    setloading(true);
    axios({
      url: `${BASE_URL}${uiux}/${id}`,
      method: "patch",
      data: {
        webImage: imageLink,
        likeCount: State.likeCount,
  ...
  useEffect(() => {
    ...
    if (router.query.id) {
      dispatch(uiuxDispatch(`/${router.query.id}`));
    }
  }, []);
  useEffect(() => {
    if (webDevState) {
      setState({
        views: webDevState.views,
        likeCount: webDevState.likeCount,
        webImage: webDevState.webImage,
